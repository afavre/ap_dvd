...
class sfImage
{
  ...
  */
  ...
  public function resizeProp($l,$h){
		$L_src=$this->getWidth();
		$H_src=$this->getHeight();
	if($L_src>$l || $H_src>$h){
		/* calcul de la hauteur pour une largeur de 47 */
		$L_ref = $l;
		$ratio1 = $L_src/$L_ref;
		$H_dst1 = $H_src/$ratio1;
					
		/* calcul de la largeur pour une hauteur de 63 */
		$H_ref = $h;
		$ratio2 = $H_src/$H_ref;
		$L_dst2 = $L_src/$ratio2;
					
		/* si les dimensions après les 2 calculs conviennent */
		if(($H_dst1 <= $H_ref) && ($L_dst2 <= $L_ref)) {
			$diff_H = $H_ref - $H_dst1;
			$diff_L = $L_ref - $L_dst2;
						
			/*Si la hauteur calculée se rapproche le plus de la hauteur voulue */
			if($diff_H <= $diff_L) {
				$hauteur_prop = $H_dst1;
				$largeur_prop = $L_ref;
			}
						
			/*Si la largeur calculée se rapproche le plus de la largeur voulue */
			else {
				$hauteur_prop = $H_ref;
				$largeur_prop = $L_dst2;
			}
		...
		/*Si le premier calcul convient seulement */
		else if(($H_dst1 <= $H_ref) && ($L_dst2 > $L_ref)) {
			$hauteur_prop = $H_dst1;
			$largeur_prop = $L_ref;
		}
					
		/*Si le deuxième calcul convient seulement */
		else if(($H_dst1 > $H_ref) && ($L_dst2 <= $L_ref)) {
			$hauteur_prop = $H_ref;
			$largeur_prop = $L_dst2;
		}
					
		$hauteur_prop = round($hauteur_prop);
		$largeur_prop = round($largeur_prop);
					
	}else{
		$largeur_prop=$L_src;
		$hauteur_prop=$H_src;
	}
    $this->resize($largeur_prop,$hauteur_prop) ;
  }
}
